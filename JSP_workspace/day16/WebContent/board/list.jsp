...
<main>
	<table class="boardList">
		...
		<tbody>
			...
			<c:if test="${empty param.search }">
				<c:set var="list" value="${boardDAO.selectBoardList(param.page) }" />
			</c:if>
	...
	<div>
		<c:if test="${empty param.column}">
			<c:set var="pageCount" value="${boardDAO.PageCount() }"/>
		
			<c:forEach var="i" begin="1" end="${pageCount }">
				<a href="${cpath }/board/board.jsp?page=${i}">[${i}]</a>
			</c:forEach>
		</c:if>
		<c:if test="${not empty param.column }">
			<c:set var="pageCount" value="${boardDAO.PageCount(param.column,param.search) }"/>
			<c:forEach var="i" begin="1" end="${pageCount }">
				<a href="${cpath }/board/board.jsp?page=${i}&column=${param.column}&search=${param.search}">[${i}]</a>
			</c:forEach>
		</c:if>
